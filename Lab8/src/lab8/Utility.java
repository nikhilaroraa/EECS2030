...
public class Utility<T extends Comparable<T>> {
    ...
    /**
     * Constructor for Utility class that accepts a list.
     *
     * @param list The list to be used by the Utility class.
     */
    ...
    public boolean containList(List<T> inputList) {
        if (this.list.containsAll(inputList)) {
            this.list.clear();
    ...
    public void removeMultipleOf10(List<? extends MyInteger> myList) {
        myList.removeIf(obj -> {
            if (obj instanceof MyInteger) {
                return MyInteger.isMultipleOf10((MyInteger) obj);
            }
            return false;
        });
    }
